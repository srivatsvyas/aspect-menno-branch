  ...
  namespace Particle
  {
    namespace Property
    {
      ...
      LpoSWaveAnisotropy<dim>::initialize ()
      {
        ...
        //std::cout << "test 1, size = " << this->get_particle_world().get_property_manager().plugin_names.size() << std::endl;
        //std::cout << this->get_particle_world().get_property_manager().plugin_names[0] << std::endl;
        //AssertThrow(this->get_particle_world().get_property_manager().plugin_name_exists("lpo"), ExcMessage("No lpo property plugin found."));
        //Assert(this->get_particle_world().get_property_manager().plugin_name_exists("lpo s wave anisotropy"), ExcMessage("No lpo property plugin found."));

        //AssertThrow(this->get_particle_world().get_property_manager().check_plugin_order("lpo","lpo s wave anisotropy"),
        ...
        for (unsigned int grain_i = 0; grain_i < n_output_grains; ++grain_i)
          {
            ...
            //std::cout << ">>> rand old = " << grain_i << ": "<< idxgrain[grain_i] << std::endl;
          }
      ...
      {
        //std::cout << "update refernce" << std::endl;
        ...
        aspect::Particle::Property::DeformationType deformation_type = DeformationType::A_type;
        ...
        switch (deformation_type)
          {
            ...
            case aspect::Particle::Property::DeformationType::A_type :
            ...
            // from Kaminski and Ribe, GJI 2004.
            case aspect::Particle::Property::DeformationType::B_type :
            ...
            // from Kaminski and Ribe, GJI 2004.
            case aspect::Particle::Property::DeformationType::C_type :
              ref_resolved_shear_stress[0] = 3;
            ...
            case aspect::Particle::Property::DeformationType::D_type :
              ref_resolved_shear_stress[0] = 1;
            ...
            case aspect::Particle::Property::DeformationType::E_type :
            ...
            case aspect::Particle::Property::DeformationType::enstatite :
        ...
        //std::cout << "ref data position = " << data_position << ":" << ref_lpo_data_position << ", ngain = " << n_grains << std::endl;
            ...
            for (size_t i = 0; i < 3; i++)
              {
                for (size_t j = 0; j < 3; j++)
                  {
                    AssertThrow(std::fabs(ref_a_cosine_matrices_olivine[grain_i][i][j] - a_cosine_matrices_olivine[grain_i][i][j]) < 1e-8,
        ...
        //std::cout << "old: n_grains = " << n_grains << ", n_samples = " << n_samples << std::endl;
        ...
        /*
        std::cout << "    old weighted_olivine_a_matrices:" << std::endl;
        ...
        for (size_t i = 0; i < eigenvector_array_a.size(); i++)
        {
          std::cout << eigenvector_array_a[0].second[i] << " ";
        }
        std::cout << std::endl;
        */
        /*
        ...
        */
        ...
        for (unsigned int i = 0; i < 3; i++)
          {
            //std::cout << i << ":" << " old = " << data[data_position + n_grains * 2.0 * (Tensor<2,3>::n_independent_components + 1) + 1 + i] << ", new = "<< averaged_a_olivine[i] << std::endl;
        ...
        /*for (size_t i = 0; i < property_information.n_fields(); i++)
        {
          unsigned int components = property_information.get_components_by_field_index(i);
          for (size_t j = 0; j < components; j++)
          {
            std::cout << i + total_components + j << " " << property_information.get_field_name_by_index(i) << " = " << data[i+total_components+j] << std::endl;
          }
          total_components += components-1;

        }*/



        std::string w_wave_anisotropy_path = this->get_output_directory() + + "w_wave_anisotropy_data/";

        if (this->get_timestep_number() == 0)
          aspect::Utilities::create_directory (w_wave_anisotropy_path,
        ...
        std::string filename = w_wave_anisotropy_path + "w_wave_anisotropy_particle_0_EA_T_" + std::to_string(timestep_number) + "." + std::to_string(dealii::Utilities::MPI::this_mpi_process (MPI_COMM_WORLD)) + ".txt";
        ...
        filename = w_wave_anisotropy_path + "w_wave_anisotropy_particle_0_weighed_EA_T_" + std::to_string(timestep_number) + "." + std::to_string(dealii::Utilities::MPI::this_mpi_process (MPI_COMM_WORLD)) + ".txt";
      ...
      LpoSWaveAnisotropy<dim>::volume_weighting(std::vector<double> fv, std::vector<std::vector<double>> angles) const
      {
        // Get volume weighted euler angles, using random draws to convert odf
        // to a discrete number of orientations, weighted by volume
        // 1a. Get index that would sort volume fractions AND
      ...
      template <int dim>
      double
      LpoSWaveAnisotropy<dim>::wrap_angle(const double angle) const
      {
      ...
      template <int dim>
      std::vector<double>
      LpoSWaveAnisotropy<dim>::extract_euler_angles_from_dcm(const Tensor<2,3> &rotation_matrix) const
      {
      ...
      template <int dim>
      Tensor<2,3>
      LpoSWaveAnisotropy<dim>::dir_cos_matrix2(double phi1, double theta, double phi2) const
      {
        Tensor<2,3> dcm;
      ...
      LpoSWaveAnisotropy<dim>::get_property_information() const
      {
        ...
        std::vector<std::pair<std::string,unsigned int> > property_information (1,std::make_pair("w_wave_anisotropy water content",1));
        for (unsigned int grain_i = 0; grain_i < n_grains; ++grain_i)
          {
            property_information.push_back(std::make_pair("w_wave_anisotropy grain " + std::to_string(grain_i) + " volume fraction olivine",1));

            for (unsigned int index = 0; index < Tensor<2,dim>::n_independent_components; index++)
              {
                property_information.push_back(std::make_pair("w_wave_anisotropy grain " + std::to_string(grain_i) + " a_cosine_matrix olivine " + std::to_string(index),1));
              }

            property_information.push_back(std::make_pair("w_wave_anisotropy grain " + std::to_string(grain_i) + " volume fraction enstatite",1));

            for (unsigned int index = 0; index < Tensor<2,dim>::n_independent_components; index++)
              {
                property_information.push_back(std::make_pair("w_wave_anisotropy grain " + std::to_string(grain_i) + " a_cosine_matrix enstatite " + std::to_string(index),1));
              }
          }
        ...
        {
          property_information.push_back(std::make_pair("w_wave_anisotropy average olivine a axis",3));
        }

        //for (unsigned int index = 0; index < 3; index++)
        {
          property_information.push_back(std::make_pair("w_wave_anisotropy average olivine b axis",3));
        }

        //for (unsigned int index = 0; index < 3; index++)
        {
          property_information.push_back(std::make_pair("w_wave_anisotropy average olivine c axis",3));
        }


        //for (unsigned int index = 0; index < 3; index++)
        {
          property_information.push_back(std::make_pair("w_wave_anisotropy average enstatite a axis",3));
        }

        //for (unsigned int index = 0; index < 3; index++)
        {
          property_information.push_back(std::make_pair("w_wave_anisotropy average enstatite b axis",3));
        }

        //for (unsigned int index = 0; index < 3; index++)
        {
          property_information.push_back(std::make_pair("w_wave_anisotropy average enstatite c axis",3));
        }

        //const std::vector<std::pair<std::string,unsigned int> > property_information (1,std::make_pair("w_wave_anisotropy a_cosine_matrix olivine " +,1));

        //const std::vector<std::pair<std::string,unsigned int> > property_information (1,std::make_pair("w_wave_anisotropy",n_grains*2*10+1));
        return property_information;
      }

      template <int dim>
      double
      LpoSWaveAnisotropy<dim>::compute_runge_kutta(std::vector<double> &volume_fractions,
      ...
      LpoSWaveAnisotropy<dim>::compute_derivatives(const std::vector<double> &volume_fractions,
