      ...
      for (unsigned int q=0; q<n_q_points; ++q)
        {
          ...
          if (theta == 0)
          ...
          else
            {
              ...
              deviator_tensor[0][0][0][0] = 0.5;
              deviator_tensor[0][0][0][1] = 0;
              deviator_tensor[0][0][1][1] = -0.5;
              deviator_tensor[0][1][0][0] = 0;
              deviator_tensor[0][1][0][1] = 1;
              deviator_tensor[0][1][1][1] = 0;
              deviator_tensor[1][1][0][0] = -0.5;
              deviator_tensor[1][1][0][1] = 0;
              deviator_tensor[1][1][1][1] = 0.5;
              //std::cout << "deviator_tensor = " << deviator_tensor << std::endl;
              //TODO: add 3d

              const SymmetricTensor<2,dim> dviscosities_dstrain_rate = derivatives->dviscosities_dstrain_rate[q];// * deviator_tensor;
                    ...
                    for (unsigned int j=0; j<stokes_dofs_per_cell; ++j)
                      {
                        data.local_matrix(i,j) += ( (eta * 2.0 * (scratch.grads_phi_u[i] * scratch.grads_phi_u[j]))
                                                    + theta * alpha * 2.0 * (scratch.grads_phi_u[i] * (dviscosities_dstrain_rate * /*deviator_tensor */ scratch.grads_phi_u[j]) * strain_rate)
                                                    //+ theta * alpha * (scratch.grads_phi_u[i] * (dviscosities_dstrain_rate * scratch.grads_phi_u[j]) * strain_rate
                                                    //                + scratch.grads_phi_u[j] * (dviscosities_dstrain_rate * scratch.grads_phi_u[i]) * strain_rate)
      ...
      for (unsigned int q=0; q<n_q_points; ++q)
        {
          ...
          if (theta == 0)
          ...
          else
            {
              ...
              //std::cout << velocity_divergence << ", ";


              for (unsigned int i=0; i<stokes_dofs_per_cell; ++i)
                for (unsigned int j=0; j<stokes_dofs_per_cell; ++j)
                  {
                    //std::cout << "dv=" << theta * two_thirds * scratch.div_phi_u[i] * ( (dviscosities_dstrain_rate * /*deviator_tensor */ scratch.grads_phi_u[j]) * velocity_divergence)
                    //<< "dp=" << theta * two_thirds * (scratch.div_phi_u[i] * dviscosities_dpressure * scratch.phi_p[j]) * velocity_divergence << std::endl;
                    data.local_matrix(i,j) += (- /*(use_tensor ?
                                                  eta_two_thirds * (scratch.div_phi_u[i] * trace(stress_strain_director * scratch.grads_phi_u[j]))
                                                  :*/
                                               eta_two_thirds * (scratch.div_phi_u[i] * scratch.div_phi_u[j])
                                               /*)*/
                                               - theta * two_thirds * scratch.div_phi_u[i] * ( (dviscosities_dstrain_rate * /*deviator_tensor */ scratch.grads_phi_u[j]) * velocity_divergence)
                                               - theta * two_thirds * (scratch.div_phi_u[i] * dviscosities_dpressure * scratch.phi_p[j]) * velocity_divergence
                                              )
                                              * JxW;
                  }
            }
