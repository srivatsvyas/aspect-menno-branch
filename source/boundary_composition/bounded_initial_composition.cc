...
#include <aspect/boundary_composition/bounded_initial_composition.h>
...
// ------------------------------ InitialComposition -------------------
    ...
    template <int dim>
    double
    BoundedInitialComposition<dim>::
    boundary_composition (const types::boundary_id /*boundary_indicator*/,
                          const Point<dim> &position,
                          const unsigned int compositional_field) const
    {
      return std::max(std::min(this->get_initial_composition_manager().initial_composition(position, compositional_field),max_composition),min_composition);
    }
    ...
    template <int dim>
    void
    BoundedInitialComposition<dim>::declare_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    BoundedInitialComposition<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Boundary composition model");
      {
        prm.enter_subsection("Initial composition");
...
namespace aspect
{
  namespace BoundaryComposition
  {
    ASPECT_REGISTER_BOUNDARY_COMPOSITION_MODEL(BoundedInitialComposition,
                                               "bounded initial composition",
