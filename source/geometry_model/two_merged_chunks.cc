...
namespace aspect
{
  namespace GeometryModel
  {
    ...
    {
      template <int dim>
      TwoMergedChunksGeometry<dim>::TwoMergedChunksGeometry()
        :
        point1_lon(0.0),
        inner_radius(3471e3),
        max_depth(2900e3)
      ...
      template <int dim>
      TwoMergedChunksGeometry<dim>::TwoMergedChunksGeometry(const TwoMergedChunksGeometry &other)
        :
      ...
      clone() const
      {
        return std_cxx14::make_unique<TwoMergedChunksGeometry>(*this);
      }
    ...
    {
      // The repetitions of each of the coarse grids.
      ...
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Chunk with lithosphere boundary indicators");
        {
          ...
          prm.declare_entry ("Chunk grid merge radius", "1",
                             ...
                             "where the two grids merge. Units: \\si{\\meter}.");
...
#define INSTANTIATE(dim) \
  template class TwoMergedChunksGeometry<dim>;
...
#undef INSTANTIATE
    ...
    ASPECT_REGISTER_GEOMETRY_MODEL(TwoMergedChunks,
                                   ...
                                   "Chunk (inner || outer || grid merge) radius: Radii at bottom and top of chunk "
                                   "and the radius at which the lower boundary indicator along a radial "
                                   "boundary transitions into the upper boundary indicator. "
                                   "(Longitude || Latitude) repetitions: "
                                   "number of cells in each coordinate direction."
                                   "(Grid merge || Outer) radius repetitions: "
                                   "number of cells in the radial coordinate direction for the lower part "
                                   "of the domain (up to the Grid merge radius) and for the upper part "
                                   "of the domain. "
                                   "\n\n"
