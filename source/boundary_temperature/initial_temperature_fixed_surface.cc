...
// ------------------------------ InitialTemperatureFixedSurface -------------------
    ...
    {
      //TimerOutput::Scope timer (this->get_computing_timer(), "Compute BC temperature");
      if (boundary_id == surface_boundary_id)
        return surface_boundary_temperature;
      else
      {
      /*tmr.reset();
      std::string key = std::to_string(position[0]) + std::to_string(position[1]) + (dim == 3 ? std::to_string(position[2]) : "");
      //std::unordered_map<std::string,double>::iterator it = value_map.find (key);
      auto it = value_map.find (key);
         double t = tmr.elapsed();
      std::cout << "key = " << key << ", in time = " << t << std::endl;
      if(it == value_map.end())
      {
	      
	  double value = this->get_initial_temperature().initial_temperature(position);
          double t2 = tmr.elapsed();
	  value_map.insert(std::make_pair(key, value));
          t = tmr.elapsed();
	  std::cout << "new: " << value <<", in time = " << t << ", t2 = " << t2 <<  std::endl; 
	  return value;
      }
      else 
      {
          t = tmr.elapsed();
	  //std::cout << "FOUND! " << (*it).second <<", in time = " << t <<  std::endl;
	  return (*it).second;
      }*/
      //tmr.reset();
	  //double value = this->get_initial_temperature().initial_temperature(position);
          //double t = tmr.elapsed();
	  //std::cout << "new: " << value <<", in time = " << t <<  std::endl; 
	  //return value;
        return this->get_initial_temperature().initial_temperature(position);
      }
    }
