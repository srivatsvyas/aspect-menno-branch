...
#include <aspect/boundary_temperature/bounded_initial_temperature.h>
...
// ------------------------------ InitialTemperature -------------------
    ...
    template <int dim>
    double
    BoundedInitialTemperature<dim>::
    boundary_temperature (const types::boundary_id,
                          const Point<dim> &position) const
    {
      return std::max(std::min(this->get_initial_temperature_manager().initial_temperature(position),max_temperature),min_temperature);
    }
    ...
    template <int dim>
    void
    BoundedInitialTemperature<dim>::declare_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    BoundedInitialTemperature<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Boundary temperature model");
      {
        prm.enter_subsection("Initial temperature");
...
namespace aspect
{
  namespace BoundaryTemperature
  {
    ASPECT_REGISTER_BOUNDARY_TEMPERATURE_MODEL(BoundedInitialTemperature,
                                               "bounded initial temperature",
