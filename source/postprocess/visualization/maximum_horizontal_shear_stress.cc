...
#include <aspect/postprocess/visualization/maximum_horizontal_shear_stress.h>
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      template <int dim>
      void
      MaximumHorizontalShearStress<dim>::
            ...
            switch (dim)
              {
                ...
                case 3:
                {
                  ...
                  const double alpha_1 = 1./2 * std::atan2 (a-b,2.0*c);
                  ...
                  const double f_double_prime_1 = 2*(a-b)*std::sin(2*alpha_1)
                                                  - 4*c*cos(2*alpha_1);
                  double alpha;
                  if (f_double_prime_1 < 0)
                  ...
                  else
                    {
                      Assert (/* f_double_prime_2 = */
                        2*(a-b)*std::sin(2*alpha_2) - 4*c*cos(2*alpha_2) <= 0,
                        ExcInternalError());
                      alpha = alpha_2;
                    }
                  ...
                  maximum_horizontal_compressive_stress
                    = n_perp * (maximum_horizontal_compressive_stress_magnitude);// -
      ...
      MaximumHorizontalShearStress<dim>::get_names () const
      {
        return std::vector<std::string> (dim, "maximum_horizontal_shear_stress");
      }


      template <int dim>
      std::vector<DataComponentInterpretation::DataComponentInterpretation>
      MaximumHorizontalShearStress<dim>::get_data_component_interpretation () const
      {
        return
          std::vector<DataComponentInterpretation::DataComponentInterpretation>
          (dim,
      ...
      template <int dim>
      UpdateFlags
      MaximumHorizontalShearStress<dim>::get_needed_update_flags () const
      {
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(MaximumHorizontalShearStress,
