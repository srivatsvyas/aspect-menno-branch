...
namespace aspect
{
  ...
  namespace MaterialModel
  {
    ...
    {
    	//return std::max(std::min(prefactor * pow(sqrt(edot_ii * edot_ii + eref * eref), alpha ),max_visc),min_visc);
    	return std::max(std::min(prefactor * pow(2*sqrt(0.5 * edot_ii * edot_ii + eref * eref), alpha ),max_visc),min_visc); // This version is needed for the nonlinear channelflow
