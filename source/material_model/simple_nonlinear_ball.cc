...
#include <aspect/material_model/simple_nonlinear_ball.h>
...
namespace aspect
{
  ...
  namespace MaterialModel
  {
	...
	compute_viscosity(const double edot_ii,const double prefactor,const double alpha, const double eref, const double min_visc, const double max_visc) const
    {
    	...
    	//std::cout << " p = " << prefactor << ", visc = " << prefactor * pow(edot_ii * edot_ii + eref * eref, alpha / 2) << std::endl;
          ...
          for (unsigned int c=0; c < volume_fractions.size(); ++c)
            {
              ...
              //std::cout << "temperature_factor = " << temperature_factor << " = (1-" << thermal_expansivities[c] << "*(" << temperature << "-" << reference_T_list[c] << ")" << std::endl;
              density += volume_fractions[c] * densities[c] * temperature_factor;
              //std::cout << "d = " << density << " += " << volume_fractions[c] << "*" << densities[c] << "*" << temperature_factor << std::endl;
            }
...
//std::cout << "HC = " << heat_capacity[0] << ":" << heat_capacity[1] << std::endl;
              ...
              for (unsigned int c=0; c < volume_fractions.size(); ++c)
                {
            	  ...
            	  const double Q = 540e3;
            	  const double Rgas = 8.314;
            	  const double local_prefactor = 0.5 * std::pow(prefactor[c],-stress_exponent_inv) * std::exp(Q / stress_exponent[c] / Rgas / temperature);
            	  //std::cout << "0.5 * " << std::pow(prefactor[c],-stress_exponent_inv) << " * " << std::exp(Q / stress_exponent[c] / Rgas / temperature) << " = "
            		//	  << "0.5 * std::pow(" << prefactor[c] << ",-" << stress_exponent_inv << ") * std::exp(" << Q << "/" << stress_exponent[c] << "/" << Rgas << "/" << temperature << ")" << std::endl;
                  composition_viscosities[c] = compute_viscosity(edot_ii,local_prefactor,alpha,eref,min_visc[c],max_visc[c]);
...
//std::cout << deriv_zero_zero << ", " << deriv_one_zero << ", " << deriv_one_one << std::endl;
 ...
 /*                 for (int x = 0; x < dim; x++)
          ...
          if(density <= 0)
        	  std::cout << "Warning density!" << std::endl;
          out.thermal_expansion_coefficients[i] = thermal_expansivity;
          // Specific heat at the given positions.
          out.specific_heat[i] = specific_heat;
          if(specific_heat <= 0)
        	  std::cout << "Warning specific_heat!" << std::endl;
          // Thermal conductivity at the given positions.
    ...
    template <int dim>
    void
	SimpleNonlinearBall<dim>::declare_parameters (ParameterHandler &prm)
    {
      ...
      {
        prm.enter_subsection ("Simple nonlinear ball");
        {
    ...
    template <int dim>
    void
	SimpleNonlinearBall<dim>::parse_parameters (ParameterHandler &prm)
    {
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(SimpleNonlinearBall,
                                   "simple nonlinear ball",
