  ...
  evaluate(const MaterialModel::MaterialModelInputs<dim> &in,
               ...
               const Point<dim> &/*position*/) const
    {
      ...
      const double strain_rate_dev_inv2 = ( (this->get_timestep_number() == 0 && strain_rate.norm() <= std::numeric_limits<double>::min())
                                            ...
                                            reference_strain_rate * reference_strain_rate
                                            :
                                            std::fabs(second_invariant(deviator(strain_rate))));//0.5 * strain_rate*strain_rate);
      ...
      const double effective_viscosity = 1.0 / ( ( 1.0 / ( viscosity + minimum_viscosity ) ) + ( 1.0 / maximum_viscosity ) );//const double effective_viscosity = std::min(std::max(viscosity,minimum_viscosity),maximum_viscosity);//1.0 / ( ( 1.0 / ( viscosity + minimum_viscosity ) ) + ( 1.0 / maximum_viscosity ) );
