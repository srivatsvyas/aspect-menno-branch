  ...
  namespace MeshRefinement
  {
    ...
    { 
      ...
      for (; cell!=endc; ++cell)
	   ...
	   for ( unsigned int it1 = 0; it1 != isoterms_i.size(); ++it1 )
		...
		if(i_excludeComposition > 0 && i_excludeComposition <= static_cast <signed int> (this->n_compositional_fields())){ // static cast to prevent compiler warnings. Will only go wrong when there are more compositional fields then a the positive part of an int can handle ;)
		   ...
		   // the temperature is between the isoterms and the exclude composition is smaller then 0.5 at this location
		   ...
		   // there is not a exclude compostion (>0) and/or the exclude composition is larger or equal to the current composition. Now we don't have to check the value of the composition anymore.
	           if(rint(in.temperature[i]) <= Utilities::string_to_int(isoterms[it1][3]) && rint(in.temperature[i]) >= Utilities::string_to_int(isoterms[it1][2])){
	            // the temperature is between the isoterms
	            // If the current refinement level is smaller or equal to the refinement minimum level, any coarsening flags should be cleared.
	            if (cell->level() <= isoterms_i[it1][0]){
                      clear_coarsen = true;
	            }
	...
	maximum_refinement_level = Utilities::string_to_int(prm.get("Initial global refinement")) + Utilities::string_to_int(prm.get("Initial adaptive refinement"));
        prm.enter_subsection("Isoterms");
        {
          ...
          // process the List of isoterms to get all data out of the structure
	  int size_element = n_isoterms_outer.size();
	  isoterms.resize(size_element,std::vector<std::string>(4,"0"));
	  // loop through all given isoterms
	  for ( unsigned int it1 = 0; it1 != n_isoterms_outer.size(); ++it1 )
	  {
	    ...
	    // The temperatures for the min and max isoterms respectively
	    isoterms[it1][2]=n_isoterms_inner[2];
	    isoterms[it1][3]=n_isoterms_inner[3];
...
namespace aspect
{
  namespace MeshRefinement
  {
    ASPECT_REGISTER_MESH_REFINEMENT_CRITERION(Isoterms,
                                              ...
                                              "two temperatures (isoterms), with the posibility"
                                              ...
                                              " isoterms'. The first parameter takes the "
