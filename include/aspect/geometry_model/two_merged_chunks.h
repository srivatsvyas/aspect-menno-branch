...
namespace aspect
{
  namespace GeometryModel
  {
    ...
    {
      /**
       * TwoMergedChunksGeometry is a class that implements the interface of
       ...
       */
      template <int dim>
      class TwoMergedChunksGeometry : public ChartManifold<dim,dim>
      {
        public:
          /**
           * Constructor
           */
          TwoMergedChunksGeometry();

          /**
           * Copy constructor
           */
          TwoMergedChunksGeometry(const TwoMergedChunksGeometry &other);
     ...
     * but with two boundary indicators per radial boundary. This allows
     * for specifying two different types of boundary conditions on the
     * lateral boundaries, for example prescribed plate motions along
     ...
     * lowernorth, uppernorth, inner and outer.
     *
     * The parameters that describe this geometry and that are read from the
     * input file are the inner outer and grid merge radii of the shell, the minimum
     ...
     * the chunk (with their boundary lying at the grid merge radius).
     *
    ...
    class TwoMergedChunks : public Interface<dim>, public SimulatorAccess<dim>
    {
      public:
         ...
         * boundary conditions.
         *
         * This geometry returns the map </code>{{"inner"->0},
         * {"outer"->1},{"lowerwest"->2}, {"lowereast"->3},
         * {"upperwest"->4}, {"uppereast"->5}}</code> in 2D,
         * and <code>{{"inner"->0},
         * {"outer"->1}, {"lowerwest"->2}, {"lowereast"->3}, {"lowersouth"->4},
         * {"lowernorth"->5},{"upperwest"->6}, {"uppereast"->7}, {"uppersouth"->8},
         * {"uppernorth"->9} }</code> in 3D.
         */
      ...
      private:
         ...
         * longitude-latitude-depth point. TODO
         */
        Point<dim> point1;
         ...
         */
        internal::TwoMergedChunksGeometry<dim> manifold;
